...
ENV_DST="${ENV_DST:-/etc/local-ai-ds4-nvidia.env}"
SERVICE_DST="/etc/systemd/system/local-ai-ds4-nvidia.service"
...
usage() {
  cat <<'USAGE'
Usage:
  sudo bash scripts/90_install_ds4_nvidia_stack.sh [options]
...
Options:
  ...
  --port PORT                service port, default 8082
  ...
  --cache-dir DIR            slot/KV cache dir, ideally on mounted NVMe
  --models-dir DIR           model dir, ideally on mounted NVMe
  --cuda-arch N              CMake CUDA arch, default 86 for RTX A4500 class
  --cuda-arch-nvcc ARCH      ds4 nvcc arch, default sm_86
  --cuda-visible-devices CSV default 0,1
  --gpu-layers N|auto        llama.cpp GPU offload layers, default auto
  --skip-download            install service without downloading a model
  --skip-ds4-reference       skip upstream ds4 CUDA reference build
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --cuda-arch) CUDA_ARCH="${2:?missing CUDA arch}"; shift 2 ;;
    --cuda-arch-nvcc) CUDA_ARCH_NVCC="${2:?missing nvcc arch}"; shift 2 ;;
    --cuda-visible-devices) CUDA_VISIBLE="${2:?missing CUDA devices}"; shift 2 ;;
    --gpu-layers) GPU_LAYERS="${2:?missing GPU layer count}"; shift 2 ;;
...
[ -f "$ROOT/conf/local-ai-ds4-nvidia.env" ] || { echo "Run from the repository checkout." >&2; exit 1; }
  ...
  nvidia-smi --query-gpu=index,memory.total --format=csv,noheader,nounits 2>/dev/null |
    awk -F, -v visible="$CUDA_VISIBLE" '
      ...
      }
      {
        ...
        gsub(/ /, "", idx)
        gsub(/ /, "", mem)
        if (visible == "" || wanted[idx]) total += mem
      }
      END { print total + 0 }
    '
}
...
choose_gpu_layers() {
  ...
  local vram_mib="$1"
  if [ "$vram_mib" -ge 38000 ]; then
    echo 16
  elif [ "$vram_mib" -ge 20000 ]; then
    echo 10
...
case "$VARIANT" in
  q2|uncensored-q2|q2-plain) min_gib=112; cache_k="q4_0"; cache_v="q4_0"; ctx=8192; batch=256; ubatch=64 ;;
  q2q4-mixed) min_gib=150; cache_k="q4_0"; cache_v="q4_0"; ctx=8192; batch=256; ubatch=64 ;;
  q4|q4-plain) min_gib=240; cache_k="q8_0"; cache_v="q8_0"; ctx=8192; batch=192; ubatch=48 ;;
  pro-q2) min_gib=480; cache_k="q8_0"; cache_v="q8_0"; ctx=4096; batch=128; ubatch=32 ;;
  too-small)
    cat >&2 <<EOF
...
EOF
    [ "$FORCE" -eq 1 ] || exit 1
    VARIANT="q2"; min_gib=0; cache_k="q4_0"; cache_v="q4_0"; ctx=4096; batch=128; ubatch=32
    ;;
...
command -v nvidia-smi >/dev/null 2>&1 || {
  echo "nvidia-smi not found. Install and validate the NVIDIA driver first." >&2
  exit 1
}
...
vram_mib="$(sum_visible_vram_mib)"
gpu_layers="$(choose_gpu_layers "$vram_mib")"

echo "[info] RAM: ${mem_gib} GiB, CPU threads: ${cpus}, visible VRAM: ${vram_mib} MiB"
echo "[info] selected DS4 GGUF variant: ${VARIANT}"
echo "[info] CUDA arch: ${CUDA_ARCH}, native DS4 arch: ${CUDA_ARCH_NVCC}, GPU layers: ${gpu_layers}"
...
CUDA_ARCH="$CUDA_ARCH" bash "$ROOT/scripts/12_install_llamacpp_cuda.sh"
bash "$ROOT/scripts/20_install_antirez_tools.sh"

if [ "$SKIP_DS4_REFERENCE" -eq 0 ]; then
  CUDA_ARCH_NVCC="$CUDA_ARCH_NVCC" bash "$ROOT/scripts/13_install_ds4_cuda_reference.sh"
else
  echo "[skip] ds4 CUDA reference build"
fi
...
install -m 0644 "$ROOT/systemd/local-ai-ds4-nvidia.service" "$SERVICE_DST"
...
LOCAL_AI_HOST=$HOST
LOCAL_AI_PORT=$PORT
LOCAL_AI_API_KEY_FILE=/etc/local-ai-ds4-nvidia.key
...
LOCAL_AI_KEEP=-1
LOCAL_AI_N_GPU_LAYERS=$gpu_layers
...
printf '%s\n' "$API_KEY" > /etc/local-ai-ds4-nvidia.key
chmod 0640 /etc/local-ai-ds4-nvidia.key
chown root:localai /etc/local-ai-ds4-nvidia.key

systemctl daemon-reload
systemctl enable --now local-ai-ds4-nvidia
...
for _ in $(seq 1 60); do
  if curl -fsS "http://127.0.0.1:${PORT}/health" >/dev/null 2>&1; then
    echo "[ok] local-ai-ds4-nvidia is healthy on port $PORT"
    break
  fi
  sleep 2
done
...
if ! curl -fsS "http://127.0.0.1:${PORT}/health" >/dev/null 2>&1; then
  echo "[error] service failed health; inspect: journalctl -u local-ai-ds4-nvidia -n 100 --no-pager" >&2
  exit 1
fi

echo "[ok] DS4 NVIDIA profile installed"
echo "[info] env: $ENV_DST"
echo "[info] status: ENV_FILE=$ENV_DST SERVICE=local-ai-ds4-nvidia local-ai status"
echo "[info] ask: ENV_FILE=$ENV_DST SERVICE=local-ai-ds4-nvidia bash scripts/50_ask.sh 'Rispondi solo OK'"
