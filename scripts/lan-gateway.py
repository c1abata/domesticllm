...
WEB_ROOT = Path(__file__).parents[1] / "web-lan"
WEB_ASSETS = {
    "/": (WEB_ROOT / "index.html", "text/html; charset=utf-8"),
    "/app.css": (WEB_ROOT / "app.css", "text/css; charset=utf-8"),
    "/app.js": (WEB_ROOT / "app.js", "text/javascript; charset=utf-8"),
    "/vendor/alpine-3.15.12.min.js": (WEB_ROOT / "vendor" / "alpine-3.15.12.min.js", "text/javascript; charset=utf-8"),
}
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def catalog() -> dict[str, Any]:
            return {"object": "list", "data": [{"id": name, "object": "model", "owned_by": "local",
                    "ready": profile.get("admitted", False), "label": profile.get("label", name),
                    "description": profile.get("description", ""),
                    "capabilities": profile.get("capabilities", ["testo"]),
                    "defaults": profile.get("ui_defaults", {})}
                    for name, profile in gateway.models.items()]}
        def serve_asset(self, path: str, head_only: bool = False) -> bool:
            ...
            self.send_header("Cache-Control", "no-store" if path == "/" else "public, max-age=3600")
        ...
        def do_HEAD(self) -> None:
            if self.serve_asset(self.path, head_only=True): return
        ...
        def do_GET(self) -> None:
            if self.serve_asset(self.path): return
