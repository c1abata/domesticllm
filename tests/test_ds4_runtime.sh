...
[ "$(ds4_kv_get "$lock" DS4_COMMIT)" = 54b36ed9ba42da31b24f2d1a5feb075c2475dbb1 ] || fail "DS4 commit pin"
...
[ "$(ds4_kv_get "$env_file" DS4_HOST)" = 127.0.0.1 ] || fail "loopback bind"
...
[ "$(ds4_kv_get "$env_file" DS4_SSD_STREAMING_CACHE_EXPERTS)" = 4GB ] || fail "long-prefill-safe expert cache budget"
...
grep -Fq 'IPAddressDeny=any' "$native_unit" || fail "network deny"
grep -Fq 'IPAddressAllow=localhost' "$native_unit" || fail "loopback allow"
grep -Fq 'ExecStartPost=/usr/local/libexec/local-ai/ds4-health wait' "$native_unit" || fail "health gate"
! grep -Fq '0.0.0.0' "$native_unit" || fail "wildcard bind"
